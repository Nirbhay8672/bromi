...
class ProjectsController extends Controller
{
     ...
     */
    public function createProjectStep1(Request $request)
    {
        try {
            $project = new Projects();
            $project->user_id = $project->added_by = Auth::user()->id;
            $project->builder_id = $request->builder_id;
            $project->website = $request->website;
            $project->project_name = $request->project_name;
            $project->address = $request->address;
            ...
            $project->project_status_question = $request->project_status_question;
            $project->restrictions = json_encode($request->restricted_user);
            $project->contact_details = json_encode($request->other_contact_details);
            $project->save();
            // return reponse
    ...
    /**
     * Update project in step 2
     * @param Illuminate\Http\Request $request
     * 
     * @return Project $project
     */
    public function createProjectStep2(Request $request, Projects $project)
    {
        try {
            $project->property_type = $request->propery_type;
            $project->property_category = $request->property_category;
            $project->sub_categories = $request->sub_categories;
            $project->sub_category_single = $request->sub_category_single;
            $tower_details = [
                'if_flat_or_penthouse' => $request->is_flat_or_penthouse,
                'if_office_or_retail' => $request->if_office_or_retail,
            ...
            $project->tower_details = json_encode($tower_details);
            $project->wing_details = $request->if_residential_only_wings;
            $project->unit_details = $request->if_residential_only_units;
            $project->land_plot_details = $request->if_farm_plot_land;
            $project->storage_industrial_details = $request->if_ware_cold_ind_plot;
            $project->storage_industrial_facilities = $request->storage_industrial_details;
            $project->extra_facilities = $request->extra_facilities;

            if ($request->propery_type == 87 || $request->property_category == 259 || $request->property_category == 260) {
                if ($request->propery_type == 87) {
                    $array = '[';
                    // foreach (json_decode($project->unit_details) as $unit) {
                        // if($unit->wing) {
                        // 	$array .= '[';
                        // 	$array .= $unit->wing;
                        // 	$array .= ','.$unit->saleable;
     ...
     * @return Project $project
     */
    public function createProjectStep3(Request $request, Projects $project)
    {
        try {
            // dd($project);
            $parking_details = [
            ...
            $project->parking_details = json_encode($parking_details);
