...
class PropertyController extends Controller
{
		...
		if ($request->ajax()) {
				...
				->when(!empty($request->search_enq), function ($query) use ($request, $enq) {
					if (!empty($enq)) {
						...
						if ($request->match_budget_from_type) {
							...
							// 	$query->whereRaw('CAST(REPLACE(REPLACE(JSON_EXTRACT(properties.unit_details, "$[0][4]"), ",", ""), "\"", "") AS UNSIGNED) >= ?', str_replace(',', '', $enq->budget_from))
							// 		->whereRaw('CAST(REPLACE(REPLACE(JSON_EXTRACT(properties.unit_details, "$[0][4]"), ",", ""), "\"", "") AS UNSIGNED) <= ?', str_replace(',', '', $enq->budget_to));
							// });
			...
			return DataTables::of($data)
				...
				->editColumn('property_category', function ($row) use ($dropdowns) {
					...
					$new_array = array('', 'office space', 'Co-working', 'Ground floor', '1st floor', '2nd floor', '3rd floor', 'Warehouse', 'Cold Storage', 'ind. shed', 'Commercial Land', 'Agricultural/Farm Land', 'Industrial Land', '1 rk', '1bhk', '2bhk', '3bhk', '4bhk', '4+bhk', 'Test', 'testw', '1Bed', '2Bed', '3Bed', '4Bed', '4+Bed');
					if ($row->property_for == 'Both') {
						$forr = 'Rent & Sell';
					} else {
				...
				->editColumn('price', function ($row) {
					...
					if (!empty($all_units)) {
						// dd($all_units);
						$vvv = ''; // Initialize an empty string to store the concatenated prices
						$all_units_length = count($all_units);
						foreach ($all_units as $key => $value) {
							if (!empty($value[0])) {
								if ($key > 0) {
									$vvv .= ' '.`<br>`; // Use <br> for a new line
								}
								$vvv .= $value[0];
							}
						}

						return $vvv; // Return the formatted string with HTML line breaks
					}
				...
				->addColumn('Actions2', function ($row) use ($dropdowns) {
					...
					$vvv = '';
					if (!empty($row->contact_details) && !empty(json_decode($row->contact_details))) {

						$cd = json_decode($row->contact_details);
						...
						if ($row->care_taker_name && $row->care_taker_contact) {
							$vvv = $vvv . '<br> ' . $row->care_taker_name . ' : ' . $row->care_taker_contact;
						}
					}
					$contact_info = ($vvv != "") ? $vvv : '-';
					// 	$buttons =  $buttons . '<i class="fa fa-phone-square fa-2x cursor-pointer color-code-popover" data-container="body"  data-bs-content="' . $contact_info . '" data-bs-trigger="hover focus"></i>';
				...
				->rawColumns(['select_checkbox', 'project_id', 'unit_details', 'updated_at', 'property_category', 'contact_details', 'Actions2', 'status_change'])
				->make(true);
		}
	...
	public function generateAreaDetails($row, $type, $dropdowns)
	{
		if ($type == 'Office') {
			...
			$typ = 'Constructed Salable Area';
			$area = explode('_-||-_', $row->constructed_salable_area)[0];
			...
			$typ = 'Salable Pllot Area';
			$area = explode('_-||-_', $row->salable_plot_area)[0];
			$measure = explode('_-||-_', $row->salable_plot_area)[1];
		}
		try {
			if (!empty($area) && !empty($measure)) {
				//$vv = ($typ.' | '.$area. ' | ' .$dropdowns[$measure]['name']);
				$vv = ($area . ' ' . $dropdowns[$measure]['name']);
				// dd("area",$vv);
				return $vv;
			}
		} catch (\Throwable $th) {
			...
			return DataTables::of($data)
				->addColumn('project_name', function (SharedProperty $shared) {
					if (!empty($shared->Property->Projects->project_name)) {
						return $shared->Property->Projects->project_name;
					} else {
						return 'N/A';
					}
				})
