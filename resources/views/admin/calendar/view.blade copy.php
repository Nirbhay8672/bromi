				...
				<div class="card h-100">
					<div class="card-header pb-0">
						...
						<div>
							<ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
								...
								<li class="nav-item" role="presentation">
									<button class="nav-link mx-1 {{$is_active}}" id="v-new-enquiry-tab" data-bs-toggle="pill" data-bs-target="#v-new-enquiry" type="button" role="tab" aria-controls="v-new-enquiry" aria-selected="true">New Enquiry</button>
								</li>
								@php $is_active = ''; @endphp
								@endif
								@if(isset($site_visit_scheduled))
								<li class="nav-item" role="presentation">
									<button class="nav-link mx-1 {{$is_active}}" id="v-site_visit_scheduled-tab" data-bs-toggle="pill" data-bs-target="#v-site_visit_scheduled" type="button" role="tab" aria-controls="v-site_visit_scheduled" aria-selected="true">Site Visit Scheduled</button>
								</li>
								@php $is_active = ''; @endphp
								@endif
								@if(isset($discussion_schedule))
								<li class="nav-item" role="presentation">
									<button class="nav-link mx-1 {{$is_active}}" id="v-discussion_schedule-tab" data-bs-toggle="pill" data-bs-target="#v-discussion_schedule" type="button" role="tab" aria-controls="v-discussion_schedule" aria-selected="false">Discussion Schedule</button>
								</li>
								@php $is_active = ''; @endphp
								@endif
								@if(isset($booked))
								<li class="nav-item" role="presentation">
									<button class="nav-link mx-1 {{$is_active}}" id="v-booked-tab" data-bs-toggle="pill" data-bs-target="#v-booked" type="button" role="tab" aria-controls="v-booked" aria-selected="false">Booked</button>
								</li>
								@php $is_active = ''; @endphp
								@endif
								@if(isset($lost))
								<li class="nav-item" role="presentation">
									<button class="nav-link mx-1 {{$is_active}}" id="v-lost-tab" data-bs-toggle="pill" data-bs-target="#v-lost" type="button" role="tab" aria-controls="v-lost" aria-selected="false">Lost</button>
								</li>
								@php $is_active = ''; @endphp
								@endif
							</ul>
						...
						<div class="tab-content" id="v-pills-tabContent">
							...
							@if(isset($new_enquiry))
							<div class="tab-pane fade {{$is_active}}" id="v-new-enquiry" role="tabpanel" aria-labelledby="v-new-enquiry-tab">
								<div class="row mb-3">
										...
										<div class="card-body h-100">
											<div class="row">
												<div class="form-group col-12 m-b-5 text-end">
													<i role="button" title="Edit" data-id="{{$data->id}}" onclick=showProgress(this) class="fs-22 py-2 mx-2 fa-pencil pointer fa " type="button"></i>
												</div>
												...
												<div class="form-group col-6 m-b-5">
													<div>:
														{{ $data->area_size_from . ' ' . (isset($dropdowns[$data->area_measurement]['name']) ? $dropdowns[$data->area_measurement]['name'] : '') }} to {{ $data->area_size_to . ' ' . (isset($dropdowns[$data->area_measurement]['name']) ? $dropdowns[$data->area_measurement]['name'] : '') }}
													</div>
												</div>
												<div class="form-group col-6 m-b-5">
												...
												<div class="form-group col-6 m-b-5">
													<div>: {{ ($data->budget_from)? $data->budget_from : '0' }} to {{$data->budget_to}}</div>
												</div>
												...
												<div class="form-group col-6 m-b-5">
													<div>: {{ (($data->enquiry_status)?'Active':'In Active') }}</div>
												</div>
							...
							@endif
							@if(isset($site_visit_scheduled))
							<div class="tab-pane fade {{$is_active}}" id="v-site_visit_scheduled" role="tabpanel" aria-labelledby="v-site_visit_scheduled-tab">
								<div class="form-group col-md-12 mb-2">
								...
								<div class="col-md-4 mb-3">
									<div class="card-body h-100">
										<div class="row">
											...
											<div class="form-group col-6 m-b-5">
												<div>: {{ ($data->dropdowns)? $data->dropdowns->name : ''  }}</div>
											</div>
											<div class="form-group col-6 m-b-5">
												<h6><b>NFD</b></h6>
											</div>
											<div class="form-group col-6 m-b-5">
												<div>: {{ ($data->nfd)? date('d-m-Y',strtotime($data->nfd)) : '' }}</div>
											</div>
											<div class="form-group col-6 m-b-5">
												<h6><b>Remarks</b></h6>
											</div>
							...
							@endif
							@if(isset($discussion_schedule))
							<div class="tab-pane fade {{$is_active}}" id="v-discussion_schedule" role="tabpanel" aria-labelledby="v-discussion_schedule-tab">
								<div class="row mb-3">
									<div class="form-group col-md-12 mb-2">
							...
							@endif
							@if(isset($booked))
							<div class="tab-pane fade {{$is_active}}" id="v-booked" role="tabpanel" aria-labelledby="v-booked-tab">
								<div class="row mb-3">
									<div class="form-group col-md-12 mb-2">
							...
							@endif
							@if(isset($lost))
							<div class="tab-pane fade {{$is_active}}" id="v-lost" role="tabpanel" aria-labelledby="v-lost-tab">
								<div class="row mb-3">
									<div class="form-group col-md-12 mb-2">
					...
					<form class="form-bookmark needs-validation modal_form" method="post" id="progress_form" novalidate="">
						...
						<div class="row">
							...
							<div class="form-group col-md-6 m-b-20">
								...
								<input class="form-control " id="nfdDateTime" name="nfdDateTime" type="datetime-local">
							</div>
			...
			success: function(data) {
				...
				for (let i = 0; i < data.length; i++) {
					...
					if (CheckIfarraykeyExists(data, i, 'user', 'first_name')) {
						str += '   <td>' + data[i]['user']['first_name'] + ' ' + data[i]['user']['last_name'] +
							'</td>';
					} else {
						str += '<td></td>';
					}
